...
class PlainVoxels(nn.Module):
    def __init__(self, voxel_size, num_grids, device):
        super().__init__()
        print(f"voxel_size={voxel_size}")
        print(f"num_grids={num_grids}")
        self.grid = UnBoundedSparseDenseGrid(
            in_dim=3,
            num_embeddings=num_grids,
            embedding_dim=5,
            grid_dim=8,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_grids", type=int, default=80000, help="capacity of sparse grids")
    ...
    o3d.io.write_line_set(f"logs/{scene}/bbox_pruned.ply", bbox_lineset)
    o3d.io.write_triangle_mesh(f"logs/{scene}/mesh_pruned.ply", mesh.to_legacy())
    o3d.io.write_line_set(f"logs/{scene}/occ_lineset_pruned.ply", model.occupancy_lineset().to_legacy())
