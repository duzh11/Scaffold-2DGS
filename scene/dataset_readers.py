...
def readColmapCameras(cam_extrinsics, cam_intrinsics, depths_params, images_folder, depths_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, depth_params=depth_params,
                              image=image, image_path=image_path, image_name=image_name, depth_path=depth_path,
...
def readColmapSceneInfo(path, images, depths, eval, lod, llffhold=8):
    try:
    ...
    if eval:
        if lod>0:
            ...
            if os.path.exists(os.path.join(path, "train_test_lists.json")):
                ...
                test_cam_ids = [i.split(".")[0] for i in train_test_lists["test"]]

                train_cam_infos = [c for c in cam_infos if c.image_name in train_cam_ids]
                test_cam_infos = [c for c in cam_infos if c.image_name in test_cam_ids]
            else:
